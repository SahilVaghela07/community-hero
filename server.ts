...
async function startServer() {
  if (process.env.NODE_ENV !== 'production') {
    ...
    // GCP Production routing
    const distPath = path.join(process.cwd(), 'dist');
    app.use(express.static(distPath));
    
    // Catch-all route to serve React app for non-API requests only
    app.get('*', (req, res, next) => {
      if (!req.path.startsWith('/api')) {
        res.sendFile(path.join(distPath, 'index.html'));
      } else {
        next(); // Let it fall through to a standard 404 for broken API routes
      }
    });
  }

  // Google Cloud Run dynamically injects the port via process.env.PORT
